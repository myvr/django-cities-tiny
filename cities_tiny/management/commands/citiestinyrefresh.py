...
--force-import option:
    ...
    option_list = BaseCommand.option_list + (
        ...
        optparse.make_option('--force-import', action='append', default=[],
                             help='Import even if filenames matching '
								  'FORCE_IMPORT are up-to-date.'
                             ),
        optparse.make_option('--force', action='append', default=[],
                             help='Download and import if filenames matching '
								  'FORCE are up-to-date.'
                             ),
    )
    ...
    def handle(self, *args, **options):
        def prepare_file(url):
            ...
            if posixpath.splitext(basename)[1] == '.zip':
                ...
                if (not os.path.exists(dest_path2) or
                        os.stat(dest_path2).st_mtime < os.stat(
                            dest_path).st_mtime):
        ...
        sources = list(COUNTRY_SOURCES) + list(ADMIN_SOURCES) + list(
            CITY_SOURCES)
        for url in sources:
            file_name, downloaded = prepare_file(url)
            force_import = options['force_import_all'] or \
                           posixpath.basename(file_name) in options[
                               'force_import']
            if downloaded or force_import:
                if url in COUNTRY_SOURCES:
                    ...
                    log.info(
                        'Importing admin division zones from %s' % file_name)
    ...
    def extract(self, zip_path, file_name):
        ...
        log.info('Extracting %s from %s into %s' % (
    ...
    def get_country_by_code2(self, code2):
        ...
        try:
            ...
            try:
                self._country_code2_map[code2] = Country.objects.get(
                    code2=code2)
    ...
    def get_admin_by_code_list(self, country, code_list):
        ...
        try:
            ...
            try:
                self._admin_code_map[country.pk][
                    code_list] = AdminDivision.objects.by_code_list(
                    country, code_list)
        ...
        for row in self.parse(file_path):
            ...
            try:
                ...
                try:
                    ...
                    obj = AdminDivision(allow_update=True,
            ...
            if obj.allow_update or obj.allow_name_update:
                try:
                    ...
                    log.warning("AdminDivision IntegrityError: %s: country=%s, "
                                ...
                                repr(admin1), repr(admin2), repr(admin3),
                                repr(admin4))
                    transaction.savepoint_rollback(sid)
